...
<body>
    ...
    if (!userID.equals(comment.getUserID())) {
        ...
        script.println("location.href='galleryView.jsp?galleryID='");
        script.println("</script>");
    } else {
        if (request.getParameter("cmtContent") == null || request.getParameter("cmtContent").equals("")){
            ...
            if (result == -1){
                ...
                PrintWriter script = response.getWriter();
                script.println("<script>");
                script.println("location.href = 'galleryView.jsp?galleryID=" + comment.getGalleryID() + "'"); // getBbsID() -> getGalleryID()로 수정
                script.println("</script>");    
            }
        }   
    }
